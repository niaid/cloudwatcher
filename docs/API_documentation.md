...
```python
def get_ec2_uptime(self, days: int, hours: int, minutes: int) -> int
```

Get the runtime of an EC2 instance
#### Parameters:

- `logger` (`logging.logger`):  logger to use. Any object that has 'info', 'warning' and 'error' methods
...
- `namespace` (`str`):  namespace of the metric, e.g. 'NepheleNamespace'
- `ec2_resource` (`boto3.resource`):  boto3 resource object to use, optional
